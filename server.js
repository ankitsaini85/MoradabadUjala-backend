...
app.get('/sitemap.xml', async (req, res) => {
  try {
    const baseUrl = process.env.CLIENT_URL || 'https://moradabadujala.in';
    ...
    // Add category pages
    const categories = ['politics', 'sports', 'entertainment', 'business', 'technology', 'health', 'ujala'];
    ...
    allNews.forEach(news => {
      if (news.slug) {
        sitemap += '  <url>\n';
        sitemap += `    <loc>${baseUrl}/news/${news.slug}</loc>\n`;
        sitemap += `    <lastmod>${new Date(news.updatedAt).toISOString()}</lastmod>\n`;
